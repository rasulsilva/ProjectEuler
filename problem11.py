...
intarrays = [[int(i) for i in M] for M in [numlist.split() for numlist in lines]]


if __name__ == "__main__":
    
    biggest_down_diag,biggest_up_diag,biggest_right, biggest_down = 0,0,0,0

    for i in range(0,17):#these loops cover diagonal downwards
        for j in range(0,17):
            diag_down_product = intarrays[i][j] * intarrays[i+1][j+1] * intarrays[i+2][j+2] * intarrays[i+3][j+3]
            biggest_down_diag = (diag_down_product if diag_down_product > biggest_down_diag else biggest_down_diag)
    for i in range(3,20):#these loops cover diagonal upwards
        for j in range(0,17):
            diag_up_product = intarrays[i][j] * intarrays[i-1][j+1] * intarrays[i-2][j+2] * intarrays[i-3][j+3]
            biggest_up_diag = (diag_up_product if diag_up_product > biggest_up_diag else biggest_up_diag)
    for i in range(0,20):#these loops cover right
        for j in range(0,17):
            right_product = intarrays[i][j] * intarrays[i][j+1] * intarrays[i][j+2] * intarrays[i][j+3]
            biggest_right = (right_product if right_product > biggest_right else biggest_right)
    for i in range(0,17):#these loops cover down
        for j in range(0,20):        
            down_product = intarrays[i][j] * intarrays[i+1][j] * intarrays[i+2][j] * intarrays[i+3][j]
            biggest_down = (down_product if down_product > biggest_down else biggest_down)
       
    
            
    max_value = max(biggest_down_diag,biggest_up_diag,biggest_right,biggest_down)
